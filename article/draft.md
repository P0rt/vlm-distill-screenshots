---
title: "I distilled a 7B vision model into a 2B one for reading screenshots — on a MacBook"
published: false
description: "A hands-on knowledge-distillation project: Qwen2-VL-7B → 2B for UI-screenshot understanding, trained, evaluated and benchmarked end-to-end on an M4 Pro. How much quality do you actually lose for a 2.4× speedup?"
tags: machinelearning, python, llm, deeplearning
canonical_url: https://github.com/P0rt/vlm-distill-screenshots
cover_image: https://raw.githubusercontent.com/P0rt/vlm-distill-screenshots/main/assets/quality_vs_speed.png
---

> **Code:** <https://github.com/P0rt/vlm-distill-screenshots> · **Model:** <https://huggingface.co/p00rt/qwen2-vl-2b-screenshots-distill>
...
**TL;DR.** The distilled 2B student runs **~2.4× faster**, in **~2.4× less memory**, with **3.75× fewer parameters** than the 7B teacher, and it clearly beats the *untrained* 2B baseline on the task. The quality story is more nuanced than "student ≈ teacher", and that nuance turned out to be the most interesting part.
...
> ⚠️ **Honesty box.** "Peak memory" on Apple Silicon is unified‑memory allocation, not CUDA VRAM. The quality numbers come from a deliberately **small** proof‑of‑concept (short training, 80 train / 16 test examples). Treat them as *trends*, not a benchmark result. The thing I'm confident in is the **method and the measurement harness** — both reproducible from the repo.
